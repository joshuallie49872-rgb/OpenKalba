...
CACHE_PATH = os.path.join(os.path.dirname(__file__), "cy_overlay_cache.json")
CACHE = {}
...
def main():
    ...
    for course in os.listdir(COURSES_DIR):
        ...
        cy_data = translate_any(en_data)

        with open(cy_path, "w", encoding="utf-8") as f:
            json.dump(cy_data, f, ensure_ascii=False, indent=2)

        total += 1

    print(f"✅ Welsh (cy) overlays created for {total} courses.")
