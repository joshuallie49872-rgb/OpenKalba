...
CACHE_PATH = os.path.join(os.path.dirname(__file__), "ga_overlay_cache.json")
CACHE = {}
...
def main():
    ...
    for course in os.listdir(COURSES_DIR):
        ...
        ga_path = os.path.join(overlays_dir, "ga.json")

        if not os.path.exists(en_path):
            continue
        if not os.path.exists(ga_path):
            # ensure file exists; fallback copy is okay
    ...
    print(f"✅ Irish (ga) overlays created for {total} courses.")
